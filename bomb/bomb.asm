...
0000000000400ee0 <phase_1>:
  ...
  400ee9:	e8 4a 04 00 00       	callq  401338 <strings_not_equal> ；第一个参数就是phase_1的第一个参数，第二个参数是上一行的0x402400，下面的代码大概就是：判断两个字符串是否相同，不同就引爆炸弹；那么如果要拆这个炸弹，就要输入跟0x402400所指向的字符串相同的字符串，但是这个文件里没有402400的相关信息，应该是被放到了其他文件夹里；那么只能靠gdb调试来看了：gdb打开bomb：gdb bomb, 然后输入 x/s 0x402400,显示“Border relations with Canada have never been better.”...
...
0000000000400efc <phase_2>:
  400efc:	55                   	push   %rbp
  400efd:	53                   	push   %rbx
  400efe:	48 83 ec 28          	sub    $0x28,%rsp ;sp指针向下移动了40个字节，8个指针空间
  400f02:	48 89 e6             	mov    %rsp,%rsi
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers>
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)           ；读取的第一个数一定要是1
  400f0e:	74 20                	je     400f30 <phase_2+0x34> ；读够了6个参数就跳400f30，否则炸弹爆炸
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax       
  400f1a:	01 c0                	add    %eax,%eax
  400f1c:	39 03                	cmp    %eax,(%rbx)           ；flag = 2 * data[i - 1] == data[i] 
  400f1e:	74 05                	je     400f25 <phase_2+0x29> ; if flag == 1 go to 400f25
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb> ; else boom
  400f25:	48 83 c3 04          	add    $0x4,%rbx 	     ; i += 1
  400f29:	48 39 eb             	cmp    %rbp,%rbx             ；if i != max_i goto 400f17(继续对比)
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b> ；
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40> ；else 成功拆弹
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx        ；写成c语言的话，这里是i = 1
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp	     ；将读取的最后一个数放在rbp
  400f3a:	eb db                	jmp    400f17 <phase_2+0x1b> ；跳400f17
...
0000000000400f43 <phase_3>:
  ...
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp) ;if 读取的第一个数 > 7 then boom
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax ;else eax <- 读取的第一个数
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8) ；goto *(8 * rax + 0x402470),浓浓的switch味道，加上上面限制了第一个参数不能大于7，因此怀疑0x402470存放跳转表，表中有8项；用gdb print /x *(0x402470 + 8 * 1) ...(0~8),果然跟下面的地址是对应的；然后看看下面的逻辑：以400f7c为例，先将$0xcf放rax，然后对比第二个参数是否与之相等，相等就拆弹，否则爆炸；那么答案就是有多个了，与400f7c对应的答案是 0 207
...
0000000000400fce <func4>:
  ...
  400fd2:	89 d0                	mov    %edx,%eax ；将第三个参数放eax
  400fd4:	29 f0                	sub    %esi,%eax ； 将第三个参数（eax中的副本）减去第二个参数
  400fd6:	89 c1                	mov    %eax,%ecx ； 将上一步的结果放到%ecx
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx ；右移31位（就是取出符号位）
  400fdb:	01 c8                	add    %ecx,%eax ；将上一步取出的符号位加到第三个参数与第二个参数之差处
  400fdd:	d1 f8                	sar    %eax ； 除以2
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx ；ecx <- (0xe + 0)
  400fe2:	39 f9                	cmp    %edi,%ecx ; 第一个参数小于等于(0xe + 0)则返回0
  ...
  400ff9:	7d 0c                	jge    401007 <func4+0x39> ；如果ecx比第一个参数大，则返回0
  400ffb:	8d 71 01             	lea    0x1(%rcx),%esi ；否则：第二个参数<-%rcx + 1,继续调用本函数
...
000000000040100c <phase_4>:
  ...
  401029:	83 f8 02             	cmp    $0x2,%eax ；读入两个参数，否则爆炸
  40102c:	75 07                	jne    401035 <phase_4+0x29>
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp) ；第一个参数小于等于15就跳（否则爆炸）
  401033:	76 05                	jbe    40103a <phase_4+0x2e>
  401035:	e8 00 04 00 00       	callq  40143a <explode_bomb>
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx ；给func4的第三个参数是0xe
  40103f:	be 00 00 00 00       	mov    $0x0,%esi ；给func4的第二个参数是0
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi ；给func4的第一个参数是phase_4的第一个参数
  401048:	e8 81 ff ff ff       	callq  400fce <func4>
  40104d:	85 c0                	test   %eax,%eax ； 非0则跳（爆炸）
  40104f:	75 07                	jne    401058 <phase_4+0x4c>
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp) ； 第二个参数为0就拆弹
...
0000000000401062 <phase_5>:
  401062:	53                   	push   %rbx 
  401063:	48 83 ec 20          	sub    $0x20,%rsp ；sp下移32个字节
  401067:	48 89 fb             	mov    %rdi,%rbx ；第一个参数给rbx备份
  40106a:	64 48 8b 04 25 28 00 	mov    %fs:0x28,%rax ；？？？
  401071:	00 00 
  401073:	48 89 44 24 18       	mov    %rax,0x18(%rsp)
  401078:	31 c0                	xor    %eax,%eax
  40107a:	e8 9c 02 00 00       	callq  40131b <string_length>
  40107f:	83 f8 06             	cmp    $0x6,%eax ；读取不等于6个字节就爆炸
  401082:	74 4e                	je     4010d2 <phase_5+0x70>
  401084:	e8 b1 03 00 00       	callq  40143a <explode_bomb> ;爆炸
  ...
  401099:	0f b6 92 b0 24 40 00 	movzbl 0x4024b0(%rdx),%edx ；0x4024b0是一个数组的起始地址，%rdx是索引；用gdb 看看是啥：x /s  0x4024b0 => 0x4024b0 <array.3449>:	"maduiersnfotvbylSo you think you can stop the bomb with ctrl-c, do you?";那么这一关的意思就是，我要依次输入6个数字，然后这6个数字是前面这串字符的索引，最后结果要映射出"flyers"；因此这6个数是9 15 14 5 6 7
  4010a0:	88 54 04 10          	mov    %dl,0x10(%rsp,%rax,1) ；

  4010a4:	48 83 c0 01          	add    $0x1,%rax ；rax是用来计数的，遍历我要输入的字符串
  4010a8:	48 83 f8 06          	cmp    $0x6,%rax
  4010ac:	75 dd                	jne    40108b <phase_5+0x29>

  4010ae:	c6 44 24 16 00       	movb   $0x0,0x16(%rsp) ；字符串终止符号
  4010b3:	be 5e 24 40 00       	mov    $0x40245e,%esi
  4010b8:	48 8d 7c 24 10       	lea    0x10(%rsp),%rdi ；0x10(%rsp)所指的字符串要跟0x40245e的相同
  4010bd:	e8 76 02 00 00       	callq  401338 <strings_not_equal> ； 
  4010c2:	85 c0                	test   %eax,%eax ；只有当eax == eax == 0时，je才会跳转（zf == 1）
  4010c4:	74 13                	je     4010d9 <phase_5+0x77> ；0则拆弹（即如0x40245e所指字符串与0x10(%rsp)所指字符串相同则跳）
  4010c6:	e8 6f 03 00 00       	callq  40143a <explode_bomb> ；爆炸
  ...
  4010e5:	00 00 
  4010e7:	74 05                	je     4010ee <phase_5+0x8c> ；拆弹
...
0000000000401338 <strings_not_equal>:
  ...
  40133a:	55                   	push   %rbp      ；rbp：被调用者保存
  40133b:	53                   	push   %rbx      ；rbx：被调用者保存
  40133c:	48 89 fb             	mov    %rdi,%rbx ;rdi:第一个参数（传给string_length），这里是先保存一份这个参数
  40133f:	48 89 f5             	mov    %rsi,%rbp ;rsi：第二个参数（传给string_length），这里是先保存一份这个参数
  401342:	e8 d4 ff ff ff       	callq  40131b <string_length>
  401347:	41 89 c4             	mov    %eax,%r12d ；rax：返回值，这里是eax，即是32位的，应该是int
  40134a:	48 89 ef             	mov    %rbp,%rdi  ;rbp：现在存放着刚才的rsi(这个函数的第二个参数)，现在要将这个参数当成string_length的第一个参数传给string_length
  40134d:	e8 c9 ff ff ff       	callq  40131b <string_length>
  401352:	ba 01 00 00 00       	mov    $0x1,%edx  
  401357:	41 39 c4             	cmp    %eax,%r12d  ；r12d:现在装着第一次调用string_length的返回值；这一行比较两次调用string_length的返回值
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63> ；如果两个字符串长度不相同，跳转至40139b
  40135c:	0f b6 03             	movzbl (%rbx),%eax
  40135f:	84 c0                	test   %al,%al
  401361:	74 25                	je     401388 <strings_not_equal+0x50> ;如果第一个参数的长度为0就跳到401388
  401363:	3a 45 00             	cmp    0x0(%rbp),%al  ；如果两个字符串的长度相同，那么比较每个字符，直至出现不同或全都比较了一遍为止
...
000000000040145c <read_six_numbers>:
  40145c:	48 83 ec 18          	sub    $0x18,%rsp ；sp向下移了24个字节，3个指针的空间
  401460:	48 89 f2             	mov    %rsi,%rdx  ；__isoc99_sscanf的第三个参数：read_six_numbers的第二个参数所指的位置
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx  ；__isoc99_sscanf的第四个参数：read_six_numbers的第二个参数所指的位置偏置4个字节

  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax  
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp) ；__isoc99_sscanf的第八个参数：read_six_numbers的第二个参数所指的位置偏置20个字节

  401470:	48 8d 46 10          	lea    0x10(%rsi),%rax
  401474:	48 89 04 24          	mov    %rax,(%rsp) ；__isoc99_sscanf的第七个参数：read_six_numbers的第二个参数所指的位置偏置16个字节
  401478:	4c 8d 4e 0c          	lea    0xc(%rsi),%r9 ；__isoc99_sscanf的第六个参数：read_six_numbers的第二个参数所指的位置偏置12个字节
  40147c:	4c 8d 46 08          	lea    0x8(%rsi),%r8  ；__isoc99_sscanf的第五个参数：read_six_numbers的第二个参数所指的位置偏置8个字节
  401480:	be c3 25 40 00       	mov    $0x4025c3,%esi  ；__isoc99_sscanf的第二个参数是0x4025c3
  401485:	b8 00 00 00 00       	mov    $0x0,%eax
  40148a:	e8 61 f7 ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  40148f:	83 f8 05             	cmp    $0x5,%eax ；__isoc99_sscanf@plt返回值大于5就可拆弹
