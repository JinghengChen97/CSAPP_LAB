...
+ 空闲块组织：隐式空闲链表（有脚部版本）
+ 放置：首次适配
+ 分割：如果请求块与选中空闲块之间相差过大则分割
+ 合并：立即合并

+ [DEBUG参考](https://www.jianshu.com/p/48d5d0554b3b)
~~~c++
/*
...
# 本次实验暴露的问题
1. 写程序的时候没有框架意识，没有理清各个函数之间的逻辑关系，导致DEBUG是玄学DEBUG，不能从全局出发思考问题所在
2. DEBUG的时候也是随缘DEBUG，感觉“”这个BUG是由这个原因造成的”然后就稍微改一下然后运行，没有思考导致这个BUG出现的多种可能的原因，导致越DE越混